<form action="{{route($models.'.update',[strtolower($model) => $row])}}" method="post" enctype="multipart/form-data">
    ...
    <div class="col-md-12 mb-4">
        ...
        <label for="e_mail">{{ucwords($input)}}</label>
        <input type="number" class="form-control" id="{{str_replace(' ','',$input)}}" name="{{str_replace(' ','',$input)}}" value="@isset($row->price){{$row->price}}@endisset">
        @error(str_replace(' ','',$input))
        <div class="invalid-feedback" style="display: block;">
            {{$message}}
        </div>
        ...
        <div class="col-md-12">
            ...
            <div id="select_menu" class="form-group mb-4">
                <select class="custom-select" name="{{$input}}">
                    <option value="on" @if(isset($row) && $row->status === 'on') selected @endif>On</option>
                    <option value="off" @if(isset($row) && $row->status === 'off') selected @endif>Off</option>
                </select>
                @error($input)
